...
test("recoverable GitHub account-link failure verifies email then restarts OAuth", async () => {
  ...
  try {
    ...
    const social = client.requests.find((request) => request.url === `${AUTH_BASE}/sign-in/social`);
    assert.ok(social);
    assert.deepEqual(JSON.parse(social.options.body), {
      provider: "github",
      callbackURL: "https://odin.example/login?returnTo=%2Fapp%3Fmode%3Dcoding",
      errorCallbackURL: "https://odin.example/login?returnTo=%2Fapp%3Fmode%3Dcoding&oauth=error",
...
test("GitHub recovery keeps redirect validation fail-closed", async () => {
