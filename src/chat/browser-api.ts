...
export async function browserApiHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    const browser = new BrowserProductStore(
      new NeonActorDatabase(services.pool, identity),
      identity.id,
    );
    ...
    if (sessions && method === "POST") {
      ...
      if (
        ...
        !body.allowedOrigins.every((item) => typeof item === "string")
      )
    ...
    const route =
      /^\/api\/browser\/projects\/([\w-]+)\/sessions\/([\w-]+)(?:\/(observe|prepare|approve|execute|reconcile|pause|resume|complete))?$/u.exec(
        url.pathname,
      );
    ...
    if (action === "observe") {
      ...
      if (typeof body.url !== "string")
        throw new ChatError("BROWSER_INPUT", "A page URL is required.");
    ...
    if (action === "prepare") {
      send(
        res,
        200,
        await browser.prepareAction(projectId, sessionId, actionRequest(await jsonBody(req))),
      );
      return;
    }
    if (action === "approve") {
      ...
      if (typeof body.actionId !== "string")
        throw new ChatError("BROWSER_INPUT", "Approval action ID is required.");
    ...
    if (action === "execute") {
      ...
      send(
        ...
        await browser.execute(
          ...
          sessionId,
          request,
          typeof body.actionId === "string" ? body.actionId : undefined,
        ),
      );
      return;
    }
    if (action === "reconcile") {
      ...
      if (body.outcome !== "EXECUTED" && body.outcome !== "NOT_EXECUTED")
        throw new ChatError(
          "BROWSER_INPUT",
          "Reconciliation requires verified executed/not-executed state.",
        );
      send(res, 200, {
        session: await browser.reconcile(projectId, sessionId, body.outcome as BrowserOutcome),
      });
      return;
    }
...
function actionRequest(raw: unknown): BrowserActionRequest {
  ...
  const allowed: readonly BrowserOperation[] = [
    "navigate",
    "read",
    "search",
    "extract",
  ...
  if (
    ...
    !allowed.includes(operation as BrowserOperation) ||
  ...
  if (body.fields !== undefined) {
    ...
    for (const [key, value] of Object.entries(body.fields)) {
      if (typeof value !== "string")
        throw new ChatError("BROWSER_INPUT", "Form field values must be text.");
