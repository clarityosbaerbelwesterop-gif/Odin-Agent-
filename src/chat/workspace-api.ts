...
export async function workspaceApiHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (!url.pathname.startsWith("/api/workspace/")) throw new ChatError("NOT_FOUND", "Endpoint not found.", 404);
    ...
    if (items && method === "GET") {
      send(res, 200, { items: await workspace.items(items[1] ?? "", url.searchParams.get("q") ?? "") });
      return;
    }
    ...
    if (item && method === "PUT") {
      ...
      if (typeof body.title !== "string" || typeof body.content !== "string" || typeof body.expectedVersion !== "number")
        throw new ChatError("INVALID_DOCUMENT", "Document title, content and expectedVersion are required.");
      send(
        res,
        200,
        { item: await workspace.updateDocument(item[1] ?? "", item[2] ?? "", { title: body.title, content: body.content, expectedVersion: body.expectedVersion }) },
      );
      return;
    }
    ...
    if (documents && method === "POST") {
      ...
      if (typeof body.title !== "string") throw new ChatError("INVALID_DOCUMENT", "Document title is required.");
    ...
    if (imports && method === "POST") {
      ...
      if (typeof body.filename !== "string" || typeof body.mimeType !== "string" || typeof body.content !== "string")
        throw new ChatError("INVALID_FILE", "filename, mimeType and textual content are required.");
      send(res, 201, { item: await workspace.importText(imports[1] ?? "", body as { filename: string; mimeType: string; content: string }) });
      return;
    }

    const artifact = /^\/api\/workspace\/projects\/([\w:-]+)\/artifacts\/from-run$/u.exec(url.pathname);
    if (artifact && method === "POST") {
      const body = object(await jsonBody(req), ["turnId", "title"]);
      if (typeof body.turnId !== "string") throw new ChatError("INVALID_PROVENANCE", "A Run is required.");
      send(res, 201, {
    ...
    if (layout && method === "PUT") {
      ...
      if (body.activeItemId !== null && body.activeItemId !== undefined && typeof body.activeItemId !== "string")
