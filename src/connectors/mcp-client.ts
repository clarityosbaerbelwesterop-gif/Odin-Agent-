...
function parseEventStream(text: string, expectedId: number): JsonRpcResponse {
  ...
  throw new ConnectorError(
    "MCP_PROTOCOL_ERROR",
    "MCP event stream did not contain the expected response.",
    502,
  );
}
...
async function readRpcResponse(response: Response, expectedId: number): Promise<JsonRpcResponse> {
  ...
  if (Number.isFinite(declared) && declared > MAX_RESPONSE_BYTES)
    throw new ConnectorError(
      ...
      "MCP response exceeded Odin's size limit.",
      502,
    );
...
export class RemoteMcpClient {
  ...
  constructor(
    ...
    if (
      ...
      (!accessToken || accessToken.length > 64_000 || /[\r\n]/u.test(accessToken))
    )
  ...
  async #listPages(signal?: AbortSignal): Promise<readonly RemoteMcpTool[]> {
    ...
    for (let page = 0; page < MAX_PAGES; page += 1) {
      const response = object(await this.#request("tools/list", cursor ? { cursor } : {}, signal));
  ...
  async #initializeLegacy(signal?: AbortSignal): Promise<void> {
    ...
    if (typeof result.protocolVersion !== "string")
      throw new ConnectorError(
        "MCP_PROTOCOL_ERROR",
        "Legacy MCP initialize response was invalid.",
        502,
      );
  ...
  async #notify(
    ...
    if (!response.ok && response.status !== 202 && response.status !== 204)
      throw new ConnectorError(
        "MCP_PROTOCOL_ERROR",
        `MCP notification returned HTTP ${response.status}.`,
        502,
      );
  }
  ...
  async #request(
    ...
    if (
      ...
      if (!initializing)
        throw new ConnectorError("MCP_PROTOCOL_ERROR", "MCP JSON-RPC response was invalid.", 502);
