...
function httpsUrl(value: unknown, label: string): string {
  if (typeof value !== "string" || value.length > 2000)
    throw new ConnectorError(
      ...
      `${label} is missing from OAuth metadata.`,
      502,
    );
  let url: URL;
  try {
...
async function readAuthorizationServer(
  ...
  for (const candidate of authorizationWellKnown(expectedIssuer)) {
    ...
    if (issuer !== expectedIssuer.href.replace(/\/$/u, ""))
      throw new ConnectorError(
        "AUTH_RESPONSE_INVALID",
        "OAuth issuer metadata did not match discovery.",
        502,
      );
    ...
    if (methods.length && !methods.includes("S256"))
      throw new ConnectorError(
        "AUTH_RESPONSE_INVALID",
        "OAuth server does not support PKCE S256.",
        502,
      );
...
function tokenSet(
  ...
  if (
    ...
    body.access_token.length > 64_000
  )
    throw new ConnectorError(
      "AUTH_RESPONSE_INVALID",
...
export async function refreshOAuthToken(
  ...
  if (!response.ok)
    throw new ConnectorError("TOKEN_REFRESH_FAILED", "OAuth token refresh failed.", 502);
  return tokenSet(await boundedJson(response), input.scopes);
}
