...
function cleanScopes(
  ...
  if (supported.length && unique.some((scope) => !supported.includes(scope)))
    throw new ConnectorError(
      "INVALID_INPUT",
      "A requested scope is not advertised by this connector.",
    );
...
export class ConnectorService {
    ...
    if (input.issuer) {
      ...
      if (callbackIssuer !== transaction.issuer.replace(/\/$/u, ""))
        throw new ConnectorError(
          ...
          "OAuth callback issuer did not match discovery.",
          403,
        );
    }
    try {
  ...
  async connectNoAuth(input: {
    ...
    if (!descriptor.authModes.includes("none"))
      throw new ConnectorError(
        "INVALID_INPUT",
        "This connector does not allow an unauthenticated MCP endpoint.",
      );
  ...
  async refreshTools(
    ...
    for (const entry of entries) {
      if (unique.has(entry.toolName))
        throw new ConnectorError(
          "MCP_TOOL_DENIED",
          "Remote MCP exposed colliding tool names.",
          502,
        );
  ...
  async callRemoteTool(
    ...
    if (!allowed)
      throw new ConnectorError(
        "MCP_TOOL_DENIED",
        "MCP tool was not discovered for this connection.",
        403,
      );
    if (!connection.endpoint)
      throw new ConnectorError("CONNECTION_NOT_FOUND", "Connector endpoint is missing.", 409);
  ...
  async #freshCredential(connection: ConnectorConnection) {
    ...
    if (credential.issuer && discovery.authorizationServer !== credential.issuer)
      throw new ConnectorError(
        ...
        "OAuth issuer changed since authorization.",
        502,
      );
