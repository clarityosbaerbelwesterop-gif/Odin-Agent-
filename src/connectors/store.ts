...
export class ConnectorStore {
  ...
  async deleteConnection(id: string): Promise<void> {
    await this.db.transaction(async (client) => {
      const result = await client.query("DELETE FROM odin_api.connector_connections WHERE id=$1", [
        id,
      ]);
      if (!result.rowCount)
        throw new ConnectorError("CONNECTION_NOT_FOUND", "Connector connection not found.", 404);
  ...
  async cacheTools(connectionId: string, tools: readonly ConnectorToolCacheEntry[]): Promise<void> {
    await this.db.transaction(async (client) => {
      await client.query("DELETE FROM odin_api.connector_tool_cache WHERE connection_id=$1", [
        connectionId,
      ]);
