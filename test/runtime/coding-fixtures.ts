...
export class ScriptedProvider implements ModelProvider {
  ...
  async generate(request: ModelRequest, _options?: ProviderCallOptions): Promise<ModelResponse> {
  ...
  stream(_request: ModelRequest, _options?: ProviderCallOptions): AsyncIterable<ModelStreamEvent> {
...
export class FixtureWorkspace implements RepositoryWorkspace {
  ...
  async search(input: {
    ...
    return [...this.#files.entries()]
      .filter(
        ([path]) => input.path === "." || path === input.path || path.startsWith(`${input.path}/`),
      )
  ...
  async patch(input: {
    ...
    if (this.sha(input.path) !== input.expectedSha) {
      throw new Error("Fixture optimistic SHA mismatch.");
    }
    this.patches.push({ content: input.content, expectedSha: input.expectedSha, path: input.path });
