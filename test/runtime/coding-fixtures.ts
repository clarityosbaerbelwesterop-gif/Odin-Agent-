...
export class ScriptedProvider implements ModelProvider {
  ...
  async generate(
    ...
    if (next === undefined) {
      throw new Error("Scripted provider response queue exhausted.");
    }
  ...
  stream(
    ...
    throw new Error(
      "M4 scripted provider streaming is intentionally unavailable.",
    );
  }
}
...
export class FixtureQualityRunner implements QualityCommandRunner {
  ...
  async run(
    ...
    if (commandId !== "verify") {
      throw new Error("Unknown fixture quality command.");
    }
    ...
    return {
      exitCode: 1,
      output: "fixture verify failed: add() must return a + b",
    };
  }
}
...
export function fixtureFiles(): Readonly<Record<string, string>> {
  return {
    "README.md":
      "A tiny math fixture. This file is irrelevant to the add implementation.",
