...
async function api(path, options = {}) {
  ...
  if (!response.ok)
    throw new Error(
      text(body.message) || text(body.error) || `Request failed (${response.status})`,
    );
  return body;
}
...
function renderSession() {
  ...
  $("session-state").textContent = session
    ...
    : "No browser session";
  $("companion-state").textContent = !session
    ? "Waiting for a safe scope"
    : session.state === "OUTCOME_UNKNOWN"
      ? "Checking an uncertain outcome"
      : session.state === "ACTIVE"
        ? "Working inside the approved scope"
...
function renderPage(evidence) {
  ...
  trust.textContent = "Untrusted web content";
  const url = document.createElement("span");
  url.textContent = evidence.url;
  meta.append(trust, url);
  const title = document.createElement("h2");
  title.textContent = evidence.title || new URL(evidence.url).hostname;
  const content = document.createElement("div");
  content.className = "page-text";
  content.textContent = evidence.text || "This page contained no readable text.";
  ...
  if (evidence.links?.length) {
    ...
    for (const item of evidence.links.slice(0, 12)) {
      ...
      button.type = "button";
      button.textContent = item.text || item.url;
  ...
  if (evidence.forms?.length) {
    ...
    forms.className = "page-forms";
    const heading = document.createElement("span");
    heading.className = "panel-label";
    heading.textContent = "FORMS · REVIEW BEFORE SUBMIT";
    forms.append(heading);
    evidence.forms.slice(0, 6).forEach((form, index) => {
...
function buildForm(form, index) {
  ...
  for (const field of form.fields ?? []) {
    ...
    input.type = ["email", "number", "date", "url"].includes(field.type) ? field.type : "text";
    input.addEventListener("input", () => values.set(field.name, input.value));
    card.append(label, input);
  }
  const submit = document.createElement("button");
  submit.type = "submit";
  submit.textContent = `Review submission ${index + 1}`;
  card.append(submit);
...
async function navigate(url) {
  ...
  try {
    const result = await api(projectPath("/observe"), {
      method: "POST",
      body: JSON.stringify({ url }),
    });
    state.session = result.session;
    renderSession();
    renderPage(result.evidence);
    setNotice("Page read. Its content remains untrusted evidence.");
    addActivity(`Read ${new URL(result.evidence.url).hostname}`);
  } catch (error) {
    setNotice(error.message, true);
    addActivity(`Blocked: ${error.message}`);
  } finally {
    $("current-action").textContent = "No action in progress.";
  }
}
async function prepareAction(request) {
  try {
    const prepared = await api(projectPath("/prepare"), {
      method: "POST",
      body: JSON.stringify(request),
    });
    if (prepared.approvalRequired === false) {
      setNotice("Action prepared without changing external state.");
      return;
    }
    state.prepared = { request, ...prepared };
    $("approval-action").textContent =
      `${request.operation[0].toUpperCase()}${request.operation.slice(1)}`;
...
async function approveAndExecute() {
  ...
  try {
    await api(projectPath("/approve"), {
      method: "POST",
      body: JSON.stringify({ actionId: prepared.actionId }),
    });
    const result = await api(projectPath("/execute"), {
      method: "POST",
      body: JSON.stringify({ ...prepared.request, actionId: prepared.actionId }),
    });
    state.session = result.session;
    renderSession();
    ...
    if (result.outcome === "UNKNOWN") {
      setNotice(
        "The request may have reached the site, but the outcome is unknown. Odin will not repeat it until external state is checked.",
        true,
      );
      addActivity("Outcome unknown — automatic replay blocked.");
    } else {
      setNotice("Approved action completed once.");
      addActivity(`${prepared.request.operation} completed.`);
    }
  } catch (error) {
    setNotice(error.message, true);
    addActivity(`Action stopped: ${error.message}`);
  } finally {
...
$("session-form").addEventListener("submit", async (event) => {
  ...
  const allowedOrigins = $("origins")
    ...
    .filter(Boolean);
  try {
    const result = await api(`/api/browser/projects/${encodeURIComponent(projectId)}/sessions`, {
      method: "POST",
      body: JSON.stringify({ runId, allowedOrigins, riskProfile: $("risk-profile").value }),
    });
    state.session = result.session;
    renderSession();
    sessionDialog.close();
    setNotice("Scoped browser session started. Redirects outside this scope will be blocked.");
...
$("approval-cancel").addEventListener("click", () => {
  ...
  $("approval-card").hidden = true;
  $("current-action").textContent = "No action in progress.";
  addActivity("Action cancelled before approval.");
});
